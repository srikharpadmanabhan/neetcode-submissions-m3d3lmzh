...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.ctx_length = context_length
        self.word_embs = nn.Embedding(vocab_size, model_dim)
        self.pos_embs = nn.Embedding(context_length, model_dim)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embedded_output = self.word_embs(context)
        pos_encoded = embedded_output + self.pos_embs(
            torch.arange(self.ctx_length)
        )

        ln_output = self.layer_norm(self.transformer_blks(pos_encoded))
        probs = nn.functional.softmax(
            self.linear(ln_output), dim=-1
        )
